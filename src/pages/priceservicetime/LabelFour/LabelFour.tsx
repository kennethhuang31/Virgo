...
const LabelFour: React.FC<Props> = props => {
  ...
  useEffect(() => {
    if (
      ...
      props.fetchDataByLevelFromServer(4);
    }
  }, []);
      ...
      <Col xs={12} key={index}>
        {editModel[item.id] ? (
          ...
          <Form.Group as={Row}>
            <Col xs={9}>
              <Link
                to={`/LabelFive/${item.id}/${item.name}`}
                onClick={(event: any) => {
...
export default connect(mapToStateProps, mapDispatchToProps)(LabelFour);
