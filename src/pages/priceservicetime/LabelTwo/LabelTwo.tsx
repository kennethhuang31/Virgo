...
const LabelTwo: React.FC<Props> = props => {
  ...
  useEffect(() => {
    if (
      ...
      props.fetchDataByLevelFromServer(2);
    }
  }, []);
      ...
      <Col xs={12} key={index}>
        {editModel[item.id] ? (
          ...
          <Form.Group as={Row}>
            <Col xs={9}>
              <Link
                to={`/LabelThree/${item.id}/${item.name}`}
                onClick={(event: any) => {
...
export default connect(mapToStateProps, mapDispatchToProps)(LabelTwo);
